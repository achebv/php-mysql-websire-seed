 ...
 *
 */
class ModelPgSqlDriver implements ModelDriverTemplate{
	...
	public function __construct(){
		if(!$this->link){
			$this->link = pg_connect("host=localhost port=5432 dbname=".DB_NAME." user=".DB_USER." password=".DB_PASS);
		}
	}
	...
	public function initSql($type){
		switch($type){
			...
			case 'update':
				...
				break;
			case 'findAll':
				$this->sql = "SELECT * FROM public.\"{$this->getTName()}\" ";
				break;
			case 'findComplex':
			...
			case 'find':
			default:
				$this->sql = "SELECT {$this->top} * FROM public.\"{$this->getTName()}\" ";
				break;
		}
	}
	...
	public function doSql(){
		...
		$data = pg_query($this->link, $this->getSql());
		$result = array();   
		if(gettype($data)!='boolean'){
			while (($row = pg_fetch_assoc($data))==true) {
				$result[] = $row;
			}
		//	Clean up
			pg_free_result($data);
		}else{
			die('get last insert ID');
		//	$data = pg_query( "SELECT SCOPE_IDENTITY() as PKey", $this->link);
